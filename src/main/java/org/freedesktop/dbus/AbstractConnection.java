...
public abstract class AbstractConnection {
    ...
    protected class FallbackContainer {
        ...
        public synchronized void add(String path, ExportedObject eo) {
            if (Debug.debug) {
                Debug.print(Debug.DEBUG, "Adding fallback on " + path + " of " + eo);
            }
        ...
        public synchronized ExportedObject get(String path) {
            ...
            for (String[] fbpath : fallbacks.keySet()) {
                ...
                if (Debug.debug) {
                    Debug.print(Debug.VERBOSE, "Matches " + i + " bestobject now " + bestobject);
                }
            }
            if (Debug.debug) {
                Debug.print(Debug.DEBUG, "Found fallback for " + path + " of " + bestobject);
            }
            return bestobject;
        }
    }
     ...
     *
     * @param objectpath The path to the object we are exposing. MUST be in
     * slash-notation, like "/org/freedesktop/Local", and SHOULD end with a
     * capitalised term. Only one object may be exposed on each path at any one
     * time, but an object may be exposed on several paths at once.
     ...
     *
     * @param objectprefix The path below which the fallback handles calls. MUST
     * be in slash-notation, like "/org/freedesktop/Local",
     * @param object The object to export.
     ...
     *
     * @param busname The bus name to connect to. Usually a well known bus name
     * in dot-notation (such as "org.freedesktop.local") or may be a DBus
     * address such as ":1-16".
     ...
     * the same full class name and exposed method signatures as the interface
     * the remote object is exporting.
    ...
    public <A> void callWithCallback(DBusInterface object, String m, CallbackHandler<A> callback, Object... parameters) {
        if (Debug.debug) {
            Debug.print(Debug.VERBOSE, "callWithCallback(" + object + "," + m + ", " + callback);
        }
    ...
    private void handleMessage(final MethodCall m) throws DBusException {
        ...
        if (null == o) {
            ...
            if (null != eo && null == eo.object.get()) {
                if (Debug.debug) {
                    Debug.print(Debug.INFO, "Unexporting " + m.getPath() + " implicitly");
                }
            ...
            if (null == meth) {
                try {
                    queueOutgoing(new Error(m, new DBus.Error.UnknownMethod(MessageFormat.format(t("The method `{0}.{1}' does not exist on this object."), new Object[]{m.getInterface(), m.getName()}))));
                } catch (DBusException DBe) {
                }
                return;
            }
        ...
        addRunnable(new Runnable() {
            ...
            public synchronized void run() {
                ...
                try {
                    ...
                    try {
                        conn.queueOutgoing(new Error(m, new DBusExecutionException(MessageFormat.format(t("Error Executing Method {0}.{1}: {2}"), new Object[]{m.getInterface(), m.getName(), e.getMessage()}))));
                    } catch (DBusException DBe) {
                    }
                }
            }
    ...
    private void handleMessage(final DBusSignal s) {
        ...
        for (final DBusSigHandler<? extends DBusSignal> h : v) {
            if (Debug.debug) {
                Debug.print(Debug.VERBOSE, "Adding Runnable for signal " + s + " with handler " + h);
            }
            addRunnable(new Runnable() {
                private boolean run = false;
    ...
    private void handleMessage(final Error err) {
        ...
        if (null != m) {
            ...
            synchronized (pendingCallbacks) {
                ...
                if (Debug.debug) {
                    Debug.print(Debug.VERBOSE, cbh + " = pendingCallbacks.remove(" + m + ")");
                }
            ...
            if (null != cbh) {
                ...
                if (Debug.debug) {
                    Debug.print(Debug.VERBOSE, "Adding Error Runnable with callback handler " + fcbh);
                }
                addRunnable(new Runnable() {
                    private boolean run = false;
                    ...
                    public synchronized void run() {
                        ...
                        try {
                            if (Debug.debug) {
                                Debug.print(Debug.VERBOSE, "Running Error Callback for " + err);
                            }
            ...
            if (null != cbh) {
                ...
                addRunnable(new Runnable() {
                    ...
                    public synchronized void run() {
                        ...
                        try {
                            if (Debug.debug) {
                                Debug.print(Debug.VERBOSE, "Running Callback for " + mr);
                            }
        ...
        try {
            ...
            if (m instanceof MethodCall) {
                if (0 == (m.getFlags() & Message.Flags.NO_REPLY_EXPECTED)) {
                    if (null == pendingCalls) {
                        ((MethodCall) m).setReply(new Error("org.freedesktop.DBus.Local", "org.freedesktop.DBus.Local.Disconnected", 0, "s", new Object[]{t("Disconnected")}));
                    } else {
                        synchronized (pendingCalls) {
