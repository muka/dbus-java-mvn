...
public class DBusDaemon extends Thread {
    ...
    static class Connstruct {
        ...
        public Connstruct(UnixSocket sock) throws IOException {
            this.usock = sock;
    ...
    public class DBusServer extends Thread implements DBus, DBus.Introspectable, DBus.Peer {
        ...
        public UInt32 RequestName(String name, UInt32 flags) {
            ...
            if (exists) {
                ...
                if (Debug.debug) {
                    Debug.print(Debug.WARN, "Client " + c.unique + " acquired name " + name);
                }
        ...
        private void handleMessage(Connstruct c, Message m) throws DBusException {
            ...
            if (Debug.debug) {
                Debug.print(Debug.VERBOSE, "Handling message " + m + " from " + c.unique);
            }
            ...
            try {
                ...
                try {
                    ...
                    send(c, new org.freedesktop.dbus.Error("org.freedesktop.DBus", c.unique, "org.freedesktop.DBus.Error.GeneralError", m.getSerial(), "s", t("An error occurred while calling ") + m.getName()));
                }
            } catch (NoSuchMethodException NSMe) {
                send(c, new org.freedesktop.dbus.Error("org.freedesktop.DBus", c.unique, "org.freedesktop.DBus.Error.UnknownMethod", m.getSerial(), "s", t("This service does not support ") + m.getName()));
            }
                ...
                if (null != wcs) {
                    try {
                        for (WeakReference<Connstruct> wc : wcs) {
                            ...
                            if (null != c) {
                                if (Debug.debug) {
                                    Debug.print(Debug.VERBOSE, "<localqueue> Got message " + m + " from " + c);
                                }
                                handleMessage(c, m);
                            }
                        }
                    ...
                    Debug.print(Debug.INFO, "Discarding " + m + " connection reaped");
                }
            }
            if (Debug.debug) {
                Debug.print(Debug.DEBUG, "exit");
    ...
    public class Sender extends Thread {
        ...
        public void run() {
            ...
            while (_run) {
                ...
                if (Debug.debug) {
                    Debug.print(Debug.VERBOSE, "Acquiring lock on outqueue and blocking for data");
                }
                Message m = null;
                ...
                if (null != wcs) {
                    for (WeakReference<Connstruct> wc : wcs) {
                        ...
                        if (null != c) {
                            if (Debug.debug) {
                                Debug.print(Debug.VERBOSE, "<outqueue> Got message " + m + " for " + c.unique);
                            }
                            if (Debug.debug) {
                                Debug.print(Debug.INFO, "Sending message " + m + " to " + c.unique);
                            }
                            try {
    ...
    public class Reader extends Thread {
        ...
        public void run() {
            ...
            while (_run && _lrun) {
                ...
                if (null != m) {
                    if (Debug.debug) {
                        Debug.print(Debug.INFO, "Read " + m + " from " + conn.unique);
                    }
            ...
            try {
                ...
                if (null != wcs) {
                    for (WeakReference<Connstruct> wc : wcs) {
                        ...
                        if (null != c) {
                            if (Debug.debug) {
                                Debug.print(Debug.INFO, "<inqueue> Got message " + m + " from " + c.unique);
                            }
                            ...
                            if (null == c.unique
                                ...
                                if ("org.freedesktop.DBus".equals(m.getDestination())) {
                                    ...
                                    if (m instanceof DBusSignal) {
                                        ...
                                        if (null == dest) {
                                            send(c, new Error("org.freedesktop.DBus", null, "org.freedesktop.DBus.Error.ServiceUnknown", m.getSerial(), "s", MessageFormat.format(t("The name `{0}' does not exist"), new Object[]{m.getDestination()})));
                                        } else {
                                            send(dest, m);
                                        }
                                    }
    ...
    private static void doUnix(BusAddress address) throws IOException {
        ...
        while (d._run) {
            UnixSocket s = uss.accept().socket();
